...
# Merge hdf5 files
lengths = [0]
with h5py.File("/home_export/qyx/GQA/images/gqa_{name}.hdf5".format(name=args.name)) as out:
    datasets = {}
    ...
    for i in tqdm(range(args.chunksNum)):
        with h5py.File("/home_export/qyx/GQA/images/{name}/gqa_{name}_{index}.h5".format(name=args.name, index=i)) as chunk:
...
# Update info file
with open("/home_export/qyx/GQA/images/{name}/gqa_{name}_info.json".format(name=args.name)) as infoIn:
    ...
    with open("/home_export/qyx/GQA/images/gqa_{name}_merged_info.json".format(name=args.name), "w") as infoOut:
